...
async function fetchRandomCustomer() {
  ...
  try {
    ...
    // show saved toast if prediction was logged to MongoDB
    if (data.saved) showSavedToast();
...
async function handleSaveCustomer() {
  ...
  const body = {
    ...
    pages_viewed  : parseFloat(document.getElementById('f-pages').value)  || 5,
    prev_purchases: parseFloat(document.getElementById('f-orders').value) || 2,
    discount_given: document.getElementById('f-discount').checked ? 1 : 0,
    gender        : document.getElementById('f-gender').value,
    device        : document.getElementById('f-device').value,
  };

  // include actual outcome label if user selected one
  const boughtVal = document.getElementById('f-bought').value;
  if (boughtVal !== '') body.bought_it = parseInt(boughtVal);
  ...
  try {
    ...
    status.textContent = `✓ Saved to MongoDB Atlas (id: ${data.id})`;
    status.className   = 'save-status ok';
    // reset outcome select after save
    document.getElementById('f-bought').value = '';
  } catch (err) {
    status.textContent = `✗ Save failed: ${err.message}`;
    status.className   = 'save-status err';
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<span class="save-icon">💾</span> Save to MongoDB';
...
// ─────────────────────────────────────────
//  SAVED TO MONGODB TOAST
// ─────────────────────────────────────────
let _toastTimer = null;
function showSavedToast() {
  let toast = document.getElementById('mongo-toast');
  if (!toast) {
    toast = document.createElement('div');
    toast.id = 'mongo-toast';
    toast.style.cssText = `
      position: fixed;
      bottom: 24px;
      right: 24px;
  ...
  toast.innerHTML = '🍃 Saved to MongoDB';
...
async function handleRetrain() {
  ...
  const status = document.getElementById('save-status');
  status.textContent = '⏳ Retraining models with latest MongoDB data…';
  status.className   = 'save-status ok';
  status.classList.remove('hidden');
  ...
  try {
    ...
    status.className   = 'save-status ok';
    // reload metrics so scores reflect new models
