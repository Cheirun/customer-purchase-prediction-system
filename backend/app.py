...
from database import (
    ...
    count_customers, insert_customer, get_all_customers_as_df,
    log_prediction, log_batch_predictions,
...
def _train_pipeline(skip_eda=False):
    ...
    try:
        ...
        rows_used = len(pd.read_csv('customer_data.csv')) if os.path.exists('customer_data.csv') else 1000
        log_retrain(
...
def predict():
    ...
    try:
        ...
        try:
            ...
            pass
        return jsonify({'status': 'ok', 'results': predictions, 'saved': saved})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
...
def save_customer():
    """
    Saves manually entered customer data to MongoDB Atlas.
    Accepts optional 'bought_it' label (0/1) — if provided the record
    can be used for future model retraining.
    ...
    try:
        record = {
            ...
            'annual_income' : float(body.get('annual_income', 50000)),
            'mins_on_site'  : float(body.get('mins_on_site', 10)),
            'pages_viewed'  : int(body.get('pages_viewed', 5)),
            'prev_purchases': int(body.get('prev_purchases', 2)),
            'discount_given': int(body.get('discount_given', 0)),
            'gender'        : str(body.get('gender', 'Male')),
            'device_used'   : str(body.get('device', body.get('device_used', 'Mobile'))),
            'source'        : 'manual',
        }
        # include bought_it label if provided (needed for retraining)
        if body.get('bought_it') is not None:
            record['bought_it'] = int(body['bought_it'])

        # also update the in-memory DATASET so company page reflects new data immediately
        global DATASET
        if DATASET is not None:
            new_row = record.copy()
            new_row['device_used'] = new_row.get('device_used', 'Mobile')
            DATASET = pd.concat([DATASET, pd.DataFrame([new_row])], ignore_index=True)

        new_id = insert_customer(record)
        return jsonify({'status': 'ok', 'message': 'Customer saved to MongoDB Atlas.', 'id': new_id})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
...
def retrain():
    ...
    try:
        ...
        # ---- try to pull training data from MongoDB Atlas first ----
        mongo_df = None
        try:
            mongo_df = get_all_customers_as_df()
        except Exception as e:
            print(f'[retrain] could not load from Atlas: {e}')

        if mongo_df is not None and len(mongo_df) >= 50:
            print(f'[retrain] training on {len(mongo_df)} rows from MongoDB Atlas')
            # save to CSV so the pipeline file-based functions also work
            mongo_df.to_csv('customer_data.csv', index=False)
            DATASET = mongo_df.copy()
            # fill nulls so pipeline doesn't crash
            for col_fill in ['age', 'annual_income', 'mins_on_site']:
                if col_fill in DATASET.columns:
                    DATASET[col_fill] = DATASET[col_fill].fillna(DATASET[col_fill].median())
        else:
            print('[retrain] falling back to CSV data (Atlas data insufficient or unavailable)')

        _train_pipeline(skip_eda=True)

        # reload DATASET after training (CSV may have been regenerated)
        if os.path.exists('customer_data.csv'):
            DATASET = pd.read_csv('customer_data.csv')
            DATASET['age']           = DATASET['age'].fillna(DATASET['age'].median())
            DATASET['annual_income'] = DATASET['annual_income'].fillna(DATASET['annual_income'].median())
        ...
        rows_used = len(DATASET) if DATASET is not None else 0
        return jsonify({
            'status' : 'ok',
            'message': f'Retrain complete. Models updated using {rows_used} customer records.',
            'rows_used': rows_used,
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
