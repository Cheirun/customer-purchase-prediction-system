...
def get_all_customers_as_df():
    """
    Returns ALL customer records from MongoDB as a DataFrame.
    Used by the retrain pipeline so new manually-added customers are included.
    Filters to only rows that have the required fields + bought_it label.
    """
    import pandas as pd
    col  = collections()['customers']
    ...
    # keep only rows that have a bought_it label (needed for supervised training)
    required = ['age', 'annual_income', 'mins_on_site', 'pages_viewed',
                'prev_purchases', 'discount_given', 'gender', 'device_used', 'bought_it']
    df = df.dropna(subset=['bought_it'])
    # make sure all required cols exist
    for col_name in required:
        if col_name not in df.columns:
            return None
    df = df[required].copy()
    df['bought_it'] = df['bought_it'].astype(int)
    print(f'[mongo] loaded {len(df)} customer records for retraining')
    return df
